...
export async function parseImageOrRawFile(file: File): Promise<RawParseResult> {
  ...
  let metadata: RawMetadata = {
    isRaw,
    cameraMake: isRaw ? getCameraMake(extension) : 'Standard Digital Camera',
    cameraModel: isRaw ? getCameraModel(extension) : 'Generic RGB Device',
    lens: isRaw ? getSimulatedLens(extension) : 'Standard Optical Lens',
    iso: isRaw ? 100 : 200,
    ...
    sensorDimensions: isRaw ? '35.9 x 24.0 mm (Full-Frame)' : 'Standard Sensor',
